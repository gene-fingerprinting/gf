...
from dtaidistance.dtw import F_distance

saved_model = join(const.MODEL_DIR, 'saved_model_DDTW.pkl')
...
class DDTW:
    ...
    def train(self, X_train, y_train):
        if not exists(saved_model):
            ...
            for y_i in tqdm(data['y'].unique(), desc='Electing'):
                ...
                samples['sum'] = samples['X'].apply(lambda X: sum([F_distance(X_i, X, window=self.window * min(len(X_i), len(X))) for X_i in samples['X']]))
    ...
    def _predict(self, X):
        dists = [F_distance(X_i, X, window=self.window * min(len(X_i), len(X))) for X_i in self.X]
